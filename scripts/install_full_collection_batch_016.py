...
CARDS: list[dict[str, object]] = [
    ...
    },
    {
        ...
        "sections": [
            ...
            sec("阅读指南", r"先看 Figure 1，确认 optimization confound；再读 Eqs. (10)–(14) 的 \(B_{\rm opt},\eta_{\rm opt},N_{\rm opt}\) 拟合。随后检查 0.72B/1.07B validation 和 40.1% cost claim，最后读 limitations，尤其是 constant learning rate 与 model-family boundary。"),
        ],
    ...
    },
    {
        ...
        "sections": [
            ...
            sec("背景", r"description \(U=(U_1,\ldots,U_N)\) 是对各分量 \(X_i\) 的局部随机压缩，且满足 \(U_i\perp X_{j\ne i}\mid X_i\)。总输入信息 \(I_{\rm in}=\sum_i I(X_i;U_i)\) 固定时，可比较同一压缩预算下的 total correlation 与 O-information。", r"Figure 1 将自旋耦合图、description channel、离散 subsystem 点、随机压缩云与优化边界放在同一相图中，是全文最接近统计物理“状态空间测绘”的图。"),
            sec("模型与方法", r"每个 \(p(u_i\mid x_i)\) 由 neural encoder 参数化；单分量 mutual information 用 likelihood-ratio lower bound，跨分量项用 InfoNCE。损失含预算约束 \(\gamma|I_{\rm in}-\hat I_{\rm in}|\) 与待极值的信息量；需要最小化 mutual information 时使用 adversarial estimator。", r"total correlation \(TC(U)\) 衡量联合后熵的减少；O-information \(\Omega(U)>0\) 表示 redundancy 主导，\(\Omega(U)<0\) 表示 synergy 主导。作者沿不同 \(\hat I_{\rm in}\) 重复优化以描出边界。"),
